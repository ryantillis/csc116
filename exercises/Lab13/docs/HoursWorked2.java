...
public class HoursWorked2 {
   ...
   public static void processFile(Scanner input){
      ...
      while (input.hasNextLine()){      
         ...
         int id = lineProcessor.nextInt(); //read the ID first
         String name = lineProcessor.next(); //read the name
         double totHours=0;
         while (lineProcessor.hasNext()){ //read the hours worked              	
             while (lineProcessor.hasNextDouble()){
                totHours += lineProcessor.nextDouble();
             }			
         }
         lineProcessor.close(); //close the Scanner to the line
